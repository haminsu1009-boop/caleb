...
# 돌면서 현재가로 주문한다. 봉 경계 자체는 문제가 아니다(종가↔다음시가
# 편향 +0.0016%). 문제는 폴링 지연이다 — 과매도 신호는 정의상 반등
# 직전이라 5분 늦게 사면 체계적으로 비싸게 산다.
#   5분 지연 진입 편향  +0.2252%
#   5분 지연 청산 편향  +0.0295%  (더 비싸게 팔아 이득)
#   왕복 순비용        +0.1957%p
# 폴링을 빨리해도 잘 안 줄어든다. 반등이 봉 확정 후 5분 안에 일어난다.
ROUND_TRIP = 0.40          # % 왕복 — 테이커 수수료 + 실측 체결지연 비용
