...
방향성 조건 64개 중 학습·홀드아웃 둘 다 수수료를 넘은 것은 3개였고,
그중 1위가 함정이었다.

    조건                   초과(같은 날 대비)    판정
    극단적 탐욕(≥80)        +0.000%          순수 국면
    미결제약정 급감          +0.54~0.68%      진짜지만 약함(1.2~1.7배)
    과매도(봇 규칙)          +3.7~4.4%        9~11배

극단적 탐욕은 수수료 대비 17.88배로 표의 1위였다. 그런데 같은 날에
시점을 가리지 않고 산 경우와 비교하면 초과가 **정확히 0.000%**다.
공포탐욕지수는 하루에 하나, 시장 전체에 하나이므로 "조건이 켜졌다"가
"그날이 표본에 들어왔다"와 같은 말이다. 타이밍 정보가 없다. 전체
평균과만 비교하면 상승장 프리미엄을 전략의 공으로 착각한다.

미결제약정 급감은 다르다. 초과가 0이 아니다 — 레버리지가 강제로
청산되며 계약이 사라지는 순간이 실제로 바닥 근처다. 다만 수수료
대비 1.2~1.7배라 단독으로는 쓸 수 없다.

그래서 봇의 과매도 진입에 필터로 걸어봤다. 미결제약정 데이터가
2023년부터라 기간을 맞춰야 한다 — 맞추지 않으면 2017~2026 전체인
기준선과 2023년 이후뿐인 필터 버전을 비교하게 된다.

    (2023~ 기준, 12봉 보유)
    조합                     n      승률      초과     신호유지율   총엣지
    과매도 단독            2,665  72.6%  +5.226%     100%   139.3
    + 미결제약정 급감       1,130  74.9%  +5.936%      42%    67.1
    + 테이커 매수비율 극단↓    487  70.6%  +6.289%      18%    30.6
    + 펀딩비 극단+           196  74.0%  +6.854%       7%    13.4

거래당 13% 좋아지자고 신호의 58%를 버린다. 총합으로는 절반 이하다.
이 세션 내내 나온 것과 같은 패턴이다 — 조건을 더하면 거래당은
올라가고 빈도는 더 크게 떨어진다.

기간을 맞추기 전에는 과매도 단독의 승률이 60.9%로 보였다. 그건
변동성이 컸던 초창기가 섞인 값이고, 맞추면 72.6%다. 그 차이를 필터
효과로 착각할 뻔했다.
