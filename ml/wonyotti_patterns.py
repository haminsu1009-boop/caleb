...
# ── 재탐색 결과 (2026-09, 미래참조 수정 후) ──────────────────────
#
# run_bracket 의 미래참조를 고친 뒤 급락반등을 다시 훑었다. 문턱 4개 ×
# 괄호 5개 = 20조합, 판정 기준은 셋 — 학습 양수 · 홀드아웃 양수 ·
# 같은 괄호를 씌운 무작위 진입 대비 순엣지 양수. 11/20이 통과했다.
#
# 괄호를 넓힐수록 좋아졌고 +30%/-10%는 네 문턱 전부에서 통과했다.
# 신호 자체에 정보가 있는 것은 분명하다:
#
#     같은 괄호(+30/-10) 무작위 진입  거래당 -0.349%  익절도달  8.0%
#     급락 -5% 후                  거래당 +0.871%  익절도달 20.6%
#     순엣지 +1.220%p · 익절 도달률 2.6배
#
# 추세 필터도 진짜로 일을 한다(모든 문턱에서 ON > OFF):
#     -4%  0.754 대 0.152   -5%  0.871 대 0.219
#     -7%  0.844 대 0.572  -10%  2.840 대 2.212
#
# 그런데 국면별로 쪼개면 정체가 드러난다.
#
#     국면    건수   비중  거래당%  승률
#     폭락    406   13%   -2.62   26%
#     하락    299   10%   -0.32   34%
...
#     급등  2,026   65%   +2.22   37%
#
# **거래의 65%가 급등장에 몰려 있고 거기서만 번다.** 신고가 돌파와
# ROC 모멘텀을 기각했던 것과 같은 서명이다. 연도별로도 2021년(+1.01,
# 1,260건)이 떠받치고 2025년 -5.78%(승률 11%)·2026년 -3.06%다.
#
# 포트폴리오에 넣으면 전체 배수는 오르지만 홀드아웃이 내려간다:
#
#     구성        전체     낙폭   샤프  1년손실  홀드아웃
#     3종       21.52배  21.6%  1.38    1%   4.94배
#     +급락 2%   21.52배  41.1%  1.31   11%   4.81배
#     +급락 5%   30.15배  25.2%  1.32   16%   3.91배
#     +급락 8%   19.17배  29.2%  1.06   21%   3.46배
#
# 비중을 올릴수록 홀드아웃이 4.94 → 4.81 → 3.91 → 3.46으로 단조
# 감소하고 1년 손실확률이 1% → 21%로 오른다. 전체 배수가 오르는 것은
# 2021년 한 해의 기여다.
#
# **기각.** 신호에 정보는 있지만 급등장 전용이고, 붙이면 홀드아웃과
# 파산 위험이 나빠진다. ml/bull_breakout.py, ml/community_strategies.py
# 의 모멘텀 계열과 같은 이유로 같은 결론이다.
