...
# 20봉은 **볼린저 상단 청산을 붙이기 전에** 정한 값이었다(당시 근거는
# ml/btc_exit_search.py, ml/sim_correct.py). 볼린저를 붙인 뒤로 시간청산의
# 역할이 바뀌었다 — 대부분의 거래는 상단에 닿아서 나가고(평균 17~24봉),
# 시간청산은 '목표에 못 닿은 거래'만 자르는 안전망이다. 그걸 20봉에서
# 자르면 반등할 시간을 안 주고 손실을 확정한다.
#
# 백테스트(ml/unified_pool.py)는 줄곧 60을 썼는데 봇만 20이었다. 그 차이가
# 작지 않다 — 3종 포트폴리오 기준:
#
#   최대보유   전체      연복리   낙폭    승률   샤프   1년손실
#   20봉      9.13배    28%    33.6%   72%   0.99    21%
#   40봉     14.39배    35%    21.6%   79%   1.22     9%
#   60봉     21.52배    41%    21.6%   81%   1.38     1%
#   80봉     20.70배    40%    21.6%   81%   1.38     2%
#
# 60에서 평평해지므로 경계에 얹힌 값이 아니다.
HOLD_BARS     = 60          # 240시간(10일). 볼린저 상단에 못 닿은 거래의 안전망.
