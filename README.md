> ℹ️ 이 저장소에는 퀀트 트레이딩 봇(아래)과는 별개로, 물류 회사 웹사이트
> (`index.html` 등)와 선박 추적 봇도 함께 들어있다.
> - `vessel/` — 파이썬 버전 (서버 상시 실행, 텔레그램+카카오+웹위젯 3채널)
> - `vessel-gas/` — 구글 앱스 스크립트 버전 (서버·시트 불필요, `/watch` 알림 등록 지원, 텔레그램 전용)
>
> 설정은 각 폴더의 README 참고.
