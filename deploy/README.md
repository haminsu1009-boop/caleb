...
오라클 클라우드의 기본 이미지는 **Oracle Linux**다. 우분투를 고를 수도
있는데, 둘은 계정명과 패키지 관리자가 다르다.

| 이미지 | 계정명 | 패키지 |
|---|---|---|
| Canonical Ubuntu | `ubuntu` | `apt` |
| Oracle Linux (기본) | `opc` | `dnf` |

`setup.sh` 는 둘 다 처리한다. systemd 유닛 파일의 `User=` 와 경로는
**Oracle Linux 기준(`opc`)으로 적혀 있으니**, 우분투를 쓰면 `opc` 를
`ubuntu` 로 바꿔라.

정확한 계정명은 오라클 콘솔의 인스턴스 상세 화면 **Username** 줄에
적혀 있다. 그게 가장 확실하다.
